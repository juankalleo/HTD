...
([visão geral oficial do TanStack Table](https://tanstack.com/table/latest/docs/overview)
— 100% de controle sobre markup e estilo, sem design próprio) ECharts entra
atrás de wrappers locais EvilCharts, então ambos respeitam o
tema institucional (`data-theme`, `--app-scale`, ver
[`../CONFIGURACAO-INSTITUCIONAL.md`](../layout/CONFIGURACAO-INSTITUCIONAL.md)) sem
...
Esse projeto não expõe `app/api/*` de uso geral — as únicas rotas no
`app/api` são as duas de relatório (`/api/relatorios/{pdf,excel}`), que
precisam de Puppeteer/ExcelJS no servidor.
...
[`(../ESTILOS-DE-PDF.md`](../arquivos/ESTILOS-DE-PDF.md), não repetido aqui.
...
[`(../SEGURANCA-EXPORTACAO.md`](../seguranca/SEGURANCA-EXPORTACAO.md).

## Checklist pra um relatório novo

1. Campos vêm só do serializer real da API — nunca inventa campo pra "ficar
   mais completo" (ver [`ADMINISTRACAO-RBAC.md`](/padrao-frontend/seguranca/administracao-rbac) pra
...
5. PDF/Excel (se houver) é gerado no servidor via `lib/server/
   ...
   [`(../ESTILOS-DE-PDF.md`](../arquivos/ESTILOS-DE-PDF.md)/[`(../ESTILOS-DE-EXCEL.md`](../arquivos/ESTILOS-DE-EXCEL.md));
