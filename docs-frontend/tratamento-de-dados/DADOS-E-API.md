...
- **`schemas/`** valida só o que o backend também valida — nunca uma regra
  ...
  só exige presence). Ver [`FORMULARIOS.md`](../formularios/FORMULARIOS.md).
...
- [TanStack Query — Overview (docs)](https://tanstack.com/query/latest/docs/framework/react/overview) — por que `fetch` fica em `services/` e o componente fala via hook.
- [React Query as a State Manager (TkDodo)](https://tkdodo.eu/blog/react-query-as-a-state-manager) — cache de server state como fonte de verdade.
- [Zod — Docs](https://zod.dev) — schema de validação espelhado do backend.
