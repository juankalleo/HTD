...
Padrão que surgiu da Identidade institucional (ícone do sistema, imagem de
fundo do login — ver
[`CONFIGURACAO-INSTITUCIONAL.md`](../layout/CONFIGURACAO-INSTITUCIONAL.md)), mas
...
- [react-easy-crop — GitHub](https://github.com/ricardo-ch/react-easy-crop) — lib headless usada no recorte (por que headless: encaixa no DaisyUI sem segundo design system).
- [MDN — File API](https://developer.mozilla.org/en-US/docs/Web/API/File_API) — `File`, `Blob` e `URL.createObjectURL` usados no preview.
- [Next.js — Image component](https://nextjs.org/docs/app/api-reference/components/image) — por que host dinâmico da API leva ao `<img>` consciente.
