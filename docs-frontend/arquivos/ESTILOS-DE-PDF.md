...
> visual de relatório sem duplicar o motor de geração.
...
feito contra a rota) em [`SEGURANCA-EXPORTACAO.md`](../seguranca/SEGURANCA-EXPORTACAO.md). Detalhe da
rota em si (por que é a única exceção a "sem `app/api/*`") em
[`relatorios/ARQUITETURA.md`](../relatorios/ARQUITETURA.md).

Toda tela de exportar PDF abre uma pré-visualização (`PdfPreviewModal`,
`shared/ui/relatorios/`) antes de baixar de verdade — nunca dispara o
download direto no clique. Ver [`SEGURANCA-EXPORTACAO.md`](../seguranca/SEGURANCA-EXPORTACAO.md).
...
institucional — ver [`CONFIGURACAO-INSTITUCIONAL.md`](../layout/CONFIGURACAO-INSTITUCIONAL.md)),
...
```ts
// features/relatorios/usuarios/hooks/use-exportar-relatorio-usuarios-pdf.ts
...
- [Puppeteer — Docs](https://pptr.dev) — motor de renderização do PDF (`page.pdf`, singleton de browser).
- [Puppeteer — Sandbox/configuration](https://pptr.dev/guides/configuration) — por que não usar `--no-sandbox`.
- [OWASP — XSS Prevention Cheat Sheet](https://cheatsheetseries.owasp.org/cheatsheets/Cross_Site_Scripting_Prevention_Cheat_Sheet.html) — escape de todo dado interpolado no HTML do template.
