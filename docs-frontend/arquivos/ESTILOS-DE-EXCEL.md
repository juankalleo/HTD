...
[`SEGURANCA-EXPORTACAO.md`](../seguranca/SEGURANCA-EXPORTACAO.md#5-preview-é-página-própria-não-modal)
...
3. Criar o par hook + página de preview específico do relatório (ver
   [`SEGURANCA-EXPORTACAO.md`](../seguranca/SEGURANCA-EXPORTACAO.md), "Um arquivo por
...
3. Todo valor de célula vindo de `dados.rows`/`dados.columns` passa por
   ...
   [`SEGURANCA-EXPORTACAO.md`](../seguranca/SEGURANCA-EXPORTACAO.md#1b-injeção-de-fórmula--ameaça-específica-de-planilha-excel)).
4. Documentar aqui (nova seção em "Os estilos disponíveis") o que o
   estilo é, quando usar.

## Leitura de apoio

- [ExcelJS — GitHub](https://github.com/exceljs/exceljs) — geração de `.xlsx` com estilo de célula open source (por que não SheetJS CE).
- [OWASP — CSV Injection](https://owasp.org/www-community/attacks/CSV_Injection) — por que `valorCelulaSegura` prefixa `'`.
