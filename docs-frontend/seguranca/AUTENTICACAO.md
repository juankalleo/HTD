...
- [OWASP — Top 10 (A07: Identification and Authentication Failures)](https://owasp.org/www-project-top-ten/) — autenticação/sessão é um dos riscos centrais cobertos aqui.
- [Cookies vs localStorage para JWT (OpenReplay)](https://blog.openreplay.com/cookies-vs-localstorage-jwt-auth/) — por que token em `localStorage` é legível por XSS (o trade-off adotado neste projeto).
- [OWASP — XSS Prevention Cheat Sheet](https://cheatsheetseries.owasp.org/cheatsheets/Cross_Site_Scripting_Prevention_Cheat_Sheet.html) — a principal ameaça de se guardar o token no `localStorage`.
