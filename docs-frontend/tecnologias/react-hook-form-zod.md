...
Dupla pra formulário — React Hook Form controla os campos
(sem re-render a cada tecla), Zod valida o formato dos dados; o
`@hookform/resolvers` é a cola entre os dois.

## Por que essa

Dupla consagrada pra formulário — o React Hook Form
controla os campos (sem re-render a cada tecla) e o Zod valida o formato.
Zod também descreve os dados de um jeito que já gera o tipo
TypeScript junto (`z.infer`), então schema e tipo nunca ficam
dessincronizados. Leitura de apoio:
[React Hook Form (docs oficiais)](https://react-hook-form.com/) e
[Zod (docs oficiais)](https://zod.dev/) — por que campos não-controlados
evitam re-render a cada tecla e por que o schema vira a única fonte de
verdade do tipo.

## Versão

`react-hook-form ^7.86.0`, `zod ^4.4.3`,
...
Schema (`features/autenticacao/login/schemas/login.schema.ts`):
...
Campo de senha sempre em inglês (`password`,
