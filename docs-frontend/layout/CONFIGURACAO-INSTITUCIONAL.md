...
A Configuração Institucional dá ao admin controle sobre tema/marca/tamanho
do sistema **pra todo mundo**, não só uma preferência pessoal de quem mexeu.
...
| Rota | Edita |
|---|---|
| `/config-institucional/aparencia` | tema, fonte, escala, largura da sidebar, altura da topbar, e os overrides avançados (fonte/cor de sidebar e topbar, tamanho de título, cor de borda/texto — ver [`APARENCIA-AVANCADA.md`](/padrao-frontend/estilos/aparencia-avancada)) |
...
[`UPLOAD-DE-IMAGEM.md`](../arquivos/UPLOAD-DE-IMAGEM.md).

## Leitura de apoio

- [Next.js — Font optimization (next/font)](https://nextjs.org/docs/app/building-your-application/optimizing/fonts) — CSS variables de fonte aplicadas no servidor.
- [Next.js — Server Components](https://nextjs.org/docs/app/getting-started/server-and-client-components) — por que o `layout.tsx` busca a config no servidor (sem flash).
- [Tailwind CSS v4 — Docs](https://tailwindcss.com/docs) — CSS variables (`--spacing`, `--app-scale`) que a escala multiplica.
