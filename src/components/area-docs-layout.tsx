 ...
 */
export function AreaDocsLayout({ area, children }: { area: AreaKey; children: React.ReactNode }) {
  return (
    <DocsLayout activeHref={`/${area}`} area={area} sidebarTree={getSidebarTree(area)}>
      {children}
    </DocsLayout>
  );
}
